...
#define MSG_LCD_ENDSTOPS                    _UxGT("限位開關")  //"Endstops" // Max length 8 characters
...
#define MSG_AUTO_HOME_X                     _UxGT("歸位 X")  //"Home X"
#define MSG_AUTO_HOME_Y                     _UxGT("歸位 Y")  //"Home Y"
#define MSG_AUTO_HOME_Z                     _UxGT("歸位 Z")  //"Home Z"
#define MSG_LEVEL_BED_HOMING                _UxGT("歸位 XYZ")  //"Homing XYZ"
...
#define MSG_SET_HOME_OFFSETS                _UxGT("設定原點偏移")  //"Set home offsets"
#define MSG_HOME_OFFSETS_APPLIED            _UxGT("偏移已啟用")  //"Offsets applied"
#define MSG_SET_ORIGIN                      _UxGT("設定原點")  //"Set origin"
...
#define MSG_PREHEAT_1_SETTINGS              MSG_PREHEAT_1 _UxGT(" 設定")  //MSG_PREHEAT_1 " conf"
#define MSG_PREHEAT_2                       _UxGT("預熱 ABS")  //"Preheat ABS"
...
#define MSG_PREHEAT_2_SETTINGS              MSG_PREHEAT_2 _UxGT(" 設定")  //MSG_PREHEAT_2 " conf"
...
#define MSG_VX_JERK                         _UxGT("X 軸抖動速率")  //"Vx-jerk"
#define MSG_VY_JERK                         _UxGT("Y 軸抖動速率")  //"Vy-jerk"
#define MSG_VZ_JERK                         _UxGT("Z 軸抖動速率")  //"Vz-jerk"
...
#define MSG_XSTEPS                          _UxGT("X 軸步數/mm")  //"Xsteps/mm" axis_steps_per_mm, axis steps-per-unit G92
#define MSG_YSTEPS                          _UxGT("Y 軸步數/mm")  //"Ysteps/mm"
#define MSG_ZSTEPS                          _UxGT("Z 軸步數/mm")  //"Zsteps/mm"
...
#define MSG_STORE_EEPROM                    _UxGT("保存設定")  //"Store memory"
#define MSG_LOAD_EEPROM                     _UxGT("載入設定")  //"Load memory"
...
#define MSG_CONTROL_RETRACT                 _UxGT("回抽長度 mm")  //"Retract mm" retract_length, retract length (positive mm)
#define MSG_CONTROL_RETRACT_SWAP            _UxGT("換手回抽長度 mm")  //"Swap Re.mm" retract_length_swap, swap retract length (positive mm), for extruder change
#define MSG_CONTROL_RETRACTF                _UxGT("回抽速率 mm/s")  //"Retract  V" retract_feedrate_mm_s, feedrate for retracting (mm/s)
#define MSG_CONTROL_RETRACT_ZLIFT           _UxGT("Hop mm")  //"Hop mm" retract_zlift, retract Z-lift
#define MSG_CONTROL_RETRACT_RECOVER         _UxGT("回抽恢復長度 mm")  //"UnRet +mm" retract_recover_length, additional recover length (mm, added to retract length when recovering)
#define MSG_CONTROL_RETRACT_RECOVER_SWAP    _UxGT("換手回抽恢復長度 mm")  //"S UnRet+mm" retract_recover_length_swap, additional swap recover length (mm, added to retract length when recovering from extruder change)
#define MSG_CONTROL_RETRACT_RECOVERF        _UxGT("回抽恢復後進料速率 mm/s")  //"UnRet  V" retract_recover_feedrate_mm_s, feedrate for recovering from retraction (mm/s)
...
#define MSG_ZPROBE_OUT                      _UxGT("Z 探針在熱床之外")  //"Z probe out. bed" Z probe is not within the physical limits
...
#define MSG_ENDSTOP_ABORT                   _UxGT("限位開關終止")  //"Endstop abort"
...
#define MSG_DELTA_CALIBRATE_X               _UxGT("校準 X")  //"Calibrate X"
#define MSG_DELTA_CALIBRATE_Y               _UxGT("校準 Y")  //"Calibrate Y"
#define MSG_DELTA_CALIBRATE_Z               _UxGT("校準 Z")  //"Calibrate Z"
#define MSG_DELTA_CALIBRATE_CENTER          _UxGT("校準中心")  //"Calibrate Center"

#define MSG_INFO_MENU                       _UxGT("關於印表機")  //"About Printer"
#define MSG_INFO_PRINTER_MENU               _UxGT("印表機資訊")  //"Printer Info"
#define MSG_INFO_STATS_MENU                 _UxGT("印表機統計")  //"Printer Stats"
#define MSG_INFO_BOARD_MENU                 _UxGT("主板資訊")  //"Board Info"
#define MSG_INFO_THERMISTOR_MENU            _UxGT("溫度計")  //"Thermistors"
#define MSG_INFO_EXTRUDERS                  _UxGT("擠出機")  //"Extruders"
#define MSG_INFO_BAUDRATE                   _UxGT("鮑率")  //"Baud"
